...
<a href="{{ $url }}" {{ $attributes->merge([
        'class' => 'px-4 py-3 flex items-center space-x-3 rounded transition-color duration-200 hover:text-white ' . (request()->is($activeRule) ? 'text-white bg-gray-900' : 'text-current')
    ])}}>
