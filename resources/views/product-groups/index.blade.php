...
<div class="container my-3">
    ...
    <div class="row justify-content-between">
        ...
        <div class="col-lg-auto d-flex gap-2 mb-3">
            <x-bs::button icon="plus" :title="__('Create')" wire:click="$emit('showModal', 'product-groups.save')"/>
            ...
            <div class="list-group-item list-group-item-action">
                <div class="row align-items-center">
                    <div class="col-lg mb-3 mb-lg-0">
                        <x-bs::link :label="$productGroup->name"
                            wire:click.prevent="$emit('showModal', 'product-groups.read', {{ $productGroup->id }})"/>

                        <p class="small text-muted mb-0">@displayDate($productGroup->created_at)</p>
                    </div>
                    <div class="col-lg-auto d-flex gap-2">
                        <x-bs::button icon="eye" :title="__('Read')" color="outline-primary" size="sm"
                            wire:click="$emit('showModal', 'product-groups.read', {{ $productGroup->id }})"/>

                        <x-bs::button icon="pencil-alt" :title="__('Update')" color="outline-primary" size="sm"
                            wire:click="$emit('showModal', 'product-groups.save', {{ $productGroup->id }})"/>

                        <x-bs::button icon="trash-alt" :title="__('Delete')" color="outline-primary" size="sm"
                            wire:click="delete({{ $productGroup->id }})" confirm/>
                    </div>
                </div>
            </div>
        @empty
    ...
    <x-bs::pagination :links="$productGroups"/>
</div>
